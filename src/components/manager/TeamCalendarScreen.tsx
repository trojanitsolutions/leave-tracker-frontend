...
import { useTeamCalendar, useTeamCalendarYear } from "@/hooks/useTeamCalendar";
...
export function TeamCalendarScreen() {
  ...
  const { data, isLoading, error } = useTeamCalendar(viewMode === "month", monthKey, department);
  const { data: yearData, isLoading: yearLoading, error: yearError } = useTeamCalendarYear(
    viewMode === "year",
    year,
    department,
  );
