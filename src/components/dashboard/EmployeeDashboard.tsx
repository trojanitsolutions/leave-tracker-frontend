...
export function EmployeeDashboard() {
  ...
  if (employee && isLoading) {
    return <div className="text-[13px] text-muted">Loading your dashboard…</div>;
  }
  ...
  if (employee && overview) {
    const { balance, status, currentLeave, currentExtension, recent, managerName, managerDepartment } =
      overview;
    ...
    if (status === "on_annual_leave" && currentLeave) {
      ...
      const elapsedDays = Math.round(
        (todayUTC().getTime() - parseISODateOnly(currentExtension.startDate).getTime()) /
          (24 * 60 * 60 * 1000),
      );
      const dayNumber = Math.min(totalDays, Math.max(1, elapsedDays + 1));
  ...
  // Not authenticated (or previewing the Manager/Admin "viewing as" roles) —
